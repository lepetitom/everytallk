...
<?php
    ...
    $bkRelatedSW        = ceris_single::bk_get_post_wide_option($postID, 'bk-related-sw');
    $bkSameCatSW        = ceris_single::bk_get_post_wide_option($postID, 'bk-same-cat-sw');
    ...
    if (defined('CERIS_FUNCTIONS_PLUGIN_DIR')) {
        ...
        $header_meta = array('date', 'view', 'comment');
    }else {
        $reactionSW = 0;
...
?>
<div class="site-content atbs-single-style-6 <?php echo esc_attr($stickySocialShareBar__Class);?> <?php echo esc_attr($infinity_wrapDivClass);?> <?php echo esc_attr($cerisExtension);?>">
    <div class="single-entry-wrap <?php if(($infinityScrolling != '') || ($infinityScrolling != 0)): echo 'single-infinity-container'; endif;?>">
        <div class="atbs-ceris-block atbs-ceris-block-<?php echo esc_attr($currentPostID);?> atbs-ceris-block--fullwidth single-entry <?php if(($postFormat == 'video') || ($postFormat == 'gallery')) {echo 'single-entry--video';}else {echo 'single-entry--template-2--no-sidebar';}?> single-4 element-scroll-percent single-entry--no-sidebar single-3 atbs-ceris-block--contiguous <?php if($infinity_wrapDivClass == '') echo 'single-end-infinity';?> <?php echo esc_attr($bookmarkClass);?> <?php if(($infinityScrolling != '') || ($infinityScrolling != 0)): echo 'single-infinity-inner'; endif;?>" data-url-to-load="<?php echo esc_url($infinity_PermalinkToLoad);?>" data-post-title-to-load="<?php echo esc_attr($nextPostTitle);?>" data-postid="<?php echo esc_attr($currentPostID);?>" data-wcount="<?php echo esc_attr($article_wcount);?>" data-next-wcount="<?php echo esc_attr($next_article_wcount);?>">
            <article <?php post_class();?>>
                <div class="single-content">
                    <?php 
                        if (($featuredImageSTS != 0) && (ceris_core::bk_check_has_post_thumbnail($postID))) {
                            $thumbAttrXXL = array (
                                'postID'        => $postID,
                                'thumbSize'     => 'ceris-xxl',                                
                            );
                            ...
                            <div class="atbs-ceris-block atbs-ceris-block--fullwidth atbs-ceris-block--contiguous single-billboard single-billboard--md">
                                ...
                                <div class="ceris-entry-meta-with-share-wrap">
                                    <div class="entry-meta ceris-entry-meta-with-share">
                                        ...
                                        <?php if ( function_exists( 'ceris_extension_single_share' ) ) { ?>
                                        <div class="ceris-share-socials">
                                            <span class="title-socials"><?php esc_html_e('Share','ceris') ?></span>
                            ...
                            <div class="single-body--content">
                                ...
                                <?php   
                                    if(($postFormat == 'video') || ($postFormat == 'gallery')) {
                                        if(ceris_single::bk_entry_media($postID) != null){
                                            echo '<div class="atbs-ceris-block atbs-ceris-block--fullwidth single-entry-featured-media-wrap">';
                    ...
                    switch($key) {
                        ...
                        case 'related': 
                            if(($bkRelatedSW != '') && ($bkRelatedSW != 0)) {
                                echo ceris_single::bk_related_post_wide($currentPost);
                            }
                            break;
                        ...
                        case 'same-cat': 
                            if(($bkSameCatSW != '') && ($bkSameCatSW != 0)) {
                                echo ceris_single::bk_same_category_posts_wide($currentPost);
                            }
                        break;
