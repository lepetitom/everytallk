<?php
/** delete cache */
add_action( 'after_switch_theme', 'ceris_delete_editor_styles_cache' );
add_action( 'redux/options/ceris_option/saved', 'ceris_delete_editor_styles_cache' );
add_action( 'redux/options/ceris_option/reset', 'ceris_delete_editor_styles_cache' );
add_action( 'redux/options/ceris_option/section/reset', 'ceris_delete_editor_styles_cache' );

/** load default fonts */
if ( ! function_exists( 'ceris_font_urls' ) ) {
	function ceris_font_urls() {

		$fonts_url       = '';
		$font_head  = _x( 'on', 'Poppins font: on or off', 'ceris' );
		$font_body = _x( 'on', 'Roboto font: on or off', 'ceris' );
...
/** editor fonts */
if ( ! function_exists( 'ceris_editor_font_urls' ) ) {
	function ceris_editor_font_urls() {
		if ( ! class_exists( 'ReduxFramework' ) ) {
			return ceris_font_urls();
		}
        
		$body_font    = ceris_core::bk_get_theme_option( 'body-typography' );
		$heading_font = ceris_core::bk_get_theme_option( 'heading-typography' );   
        $secondary_font = ceris_core::bk_get_theme_option( 'meta-typography' );    
		$meta_font    = ceris_core::bk_get_theme_option( 'tertiary-typography' );
...
/** dynamic editor */
if ( ! function_exists( 'ceris_editor_dynamic' ) ) {
	function ceris_editor_dynamic() {
        $ceris_font_body = ceris_core::bk_get_theme_option( 'body-typography' );
        
		if ( ! class_exists( 'ReduxFramework' ) && empty( $ceris_font_body ) ) {
			return;
		}

		$cache = get_option( 'ceris_editor_cache' );
        
		if ( 1 ) {
			$editor_css   = '';
			$body_font    = ceris_core::bk_get_theme_option( 'body-typography' );
			$heading_font = ceris_core::bk_get_theme_option( 'heading-typography' );   
            $secondary_font = ceris_core::bk_get_theme_option( 'secondary-typography' );    
			$meta_font    = ceris_core::bk_get_theme_option( 'tertiary-typography' );
			$primary_color = ceris_core::bk_get_theme_option( 'bk-primary-color' );
			...
			delete_option( 'ceris_editor_cache' );
			add_option( 'ceris_editor_cache', $cache );
		} else {
			$editor_css = stripslashes( $cache );
		}

		if ( ! empty( $editor_css ) ) {
			wp_add_inline_style( 'ceris-editor-style', $editor_css );
		}
	}
}

/** delete dynamic css */
if ( ! function_exists( 'ceris_delete_editor_styles_cache' ) ) {
	function ceris_delete_editor_styles_cache() {
		delete_option( 'ceris_editor_cache' );
