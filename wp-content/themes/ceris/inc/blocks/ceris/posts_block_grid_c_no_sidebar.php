<?php
if (!class_exists('ceris_posts_block_grid_c_no_sidebar')) {
    class ceris_posts_block_grid_c_no_sidebar {
        
        static $pageInfo=0;
        
        public function render( $page_info ) {
            $block_str = '';
            $moduleID = uniqid('ceris_posts_block_grid_c_no_sidebar-');
            ...
            if ( $the_query->have_posts()) :
                $block_str .= '<div id="'.$moduleID.'" class="atbs-ceris-block atbs-ceris-block--fullwidth atbs-ceris-block-custom-margin ceris_posts_block_grid_c_no_sidebar  atbs-ceris-featured-block-f  '.$moduleCustomClass.' '.$blockMarginTopClass.'">';
        ...
        public function render_modules ($the_query, $moduleInfo = ''){
            ...
            if ( $the_query->have_posts() ) :
                ...
                $postVerticalAttr_2 = array (
                    'additionalClass'   => 'ceris-post-vertical--cat-overlay big-post--vertical post__thumb-470'.$bypasspostAnimationDetech,
                ...
                while ( $the_query->have_posts() ): $the_query->the_post();
                    ...
                    if($currentPost == 0) {
                        ...
                        if($bypassPostIconDetech != 1) {
                            ...
                            $postIconAttr['postIconClass']  = ceris_core::get_post_icon_class($postIconAttr['iconType'], 'medium', $iconPosition_L);
                        ...
                        if($bypassPostIconDetech != 1) {
                            ...
                            if($postSource != 'all') {
                                ...
                                $postIconAttr['postIconClass']  = ceris_core::get_post_icon_class($postIconAttr['iconType'], 'small', $iconPosition_S, $addClass);
                ...
                endwhile;
                if(($currentPost == 1) || ($currentPost == 3)) {
                    $render_modules .= '</div></div> <!-- grid-top-left -->';
                }elseif(($currentPost == 2) || ($currentPost == 4)) {
                    $render_modules .= '</div> <!-- grid-top-left -->';
                }
                $render_modules .= '</div><!-- .row--space-between -->';
            endif;
